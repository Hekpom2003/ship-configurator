...
class EditFloor extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      floorGrid: {
        cols: this.props.floor.cols || 1,
        rows: this.props.floor.rows || 1,
      },
      gridOffset: this.props.floor.offset,
      gridSize: {
        width: 1055,
        height:160,
      },
  ...
  render() {
    console.log('EditFloor', this.props);
    ...
    return (
      <div className="ship-floor">
        ...
        <FloorGrid floorKey={floorKey}
                   floorGrid={this.state.floorGrid}
                   _onChangeState={obj => this.setState(obj)}/>

        <FloorPlane floorKey={floorKey}
